export const personalInfo = {
  name: 'Kevin Valencia',
  title: 'Software Engineer @ Clover | CS @ Queen\'s University',
  email: 'krvalencia06@gmail.com',
  calLink: 'https://cal.com/kevin-valencia-3utkek/15min',
  resumePath: '/resume.pdf',
  profileImage: '/5Mzl_Jju_400x400.jpg',
  bio: [
    "Hey! I'm Kevin, a Toronto-based CS student at Queen's University focused on AI and obsessed with building full-stack AI products. I absolutely love to turn ideas from 0 to 1, and I highly prioritize shipping fast, iterating faster, and scaling what works.",
    "Currently, I'm a software engineer at Clover Labs, while building a startup in the GPU infrastructure space.",
  ],
  cloverLabsLink: 'https://www.linkedin.com/posts/mattespoz_its-official-clover-labs-is-the-fastest-activity-7356360002451726338-n4V7?utm_source=share&utm_medium=member_desktop&rcm=ACoAADznI0gBv8H0a0vVr2CpHkYl7TNQwDWxpCs',
  socials: {
    github: 'https://github.com/kevinvalenciaa',
    x: 'https://x.com/kkevinvalencia',
    linkedin: 'https://www.linkedin.com/in/kevin-valenciaa/',
    substack: 'https://substack.com/@kevinvalencia',
    leetcode: 'https://leetcode.com/u/kvalencia06/',
  },
};
